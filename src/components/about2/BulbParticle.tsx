...
const vertexShader = `
  ...
  void main() {
    ...
    // Fade Bulb in and out based on uProgress
    // Bulb is visible from 0.15 to 0.85
    float fadeIn = smoothstep(0.15, 0.25, uProgress);
    float fadeOut = 1.0 - smoothstep(0.75, 0.85, uProgress);
    ...
    vAlpha *= fade; // Global fade based on scroll
...
interface BulbParticleProps {
  activeTech: string | null;
  isMobile?: boolean;
}

export default function BulbParticle({ activeTech, isMobile = false }: BulbParticleProps) {
  const materialRef = useRef<THREE.ShaderMaterial>(null);
  
  const { positions, normals, sizes, randoms } = useMemo(() => {
    // Generate only the bulb shape
    // Since it's a simpler shape, we can use far fewer particles than the brain
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (i < structureCount) {
        ...
        const radTheta = Math.atan2(snz, snx);
        
        // Distribute for targets
        positions[i3] = rBulb * Math.cos(radTheta) * 1.5; // Scale up
        positions[i3 + 1] = yBulb * 1.5;
        positions[i3 + 2] = rBulb * Math.sin(radTheta) * 1.5;
